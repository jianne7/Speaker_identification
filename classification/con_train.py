...
from model.conformer_model import SpeakerConformer, LabelSmoothingLoss
...
def main_worker(rank, args, ngpus_per_node):
    ...
    early_stop = EarlyStopping(patience = 30, verbose = True, \
                                model_path = os.path.join(SAVE_PATH, "best_train_conformer.pt"),
                                optimizer_path = os.path.join(SAVE_PATH, 'best_train_conformer_optim.pt') )
    ...
    for i in tqdm(range(epoch)):
        ...
        if rank == 0:
            writer_train.add_scalar('loss/speaker_embedding_train_v{}'.format(args.version), loss.detach().cpu().numpy(), i+1+offset)
        ...
        with torch.no_grad():
            ...
            if rank == 0:
                writer_valid.add_scalar('loss/speaker_embedding_train_v{}'.format(args.version), loss.detach().cpu().numpy(), i+1+offset)
            ...
            if rank == 0 and i % 100 == 99:
                ...
                torch.save(model.state_dict(), os.path.join(SAVE_PATH, f"{i+offset}_train_conformer.pt"))
                print('lr: {: .8f}'.format(optimizer.param_groups[0]['lr']))
                torch.save(optimizer.state_dict(), os.path.join(SAVE_PATH, f'{i+offset}_train_conformer_optim.pt'))
