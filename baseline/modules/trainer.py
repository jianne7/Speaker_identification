...
from modules.utils import AverageMeter, ProgressMeter, find_id
...
def validate_identification(cfg, model, test_loader, criterion):
    ...
    top5 = AverageMeter('Acc@5', ':6.2f')
    progress = ProgressMeter(
        len(test_loader), batch_time, losses, top1, top5, prefix='Val: ', logger=logger)
...
def predict_identification(cfg, model, test_loader, criterion, threshold):
    batch_time = AverageMeter('Time', ':6.3f')
    progress = ProgressMeter(
    ...
    with torch.no_grad():
        ...
        for i, input in (enumerate(tqdm((test_loader)))):
            ...
            prediction.append(predict(cfg, output, topk=(1, 5), threshold=threshold))
            recording.append(test_loader.dataset.features[i].stem+'.wav')
